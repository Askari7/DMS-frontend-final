...
console.log(formData,'formdata');
      ...
      try {
        ...
        const response = await axios.post(
          `https://54.81.250.98:8083/api/documents/upload`,        
          formData,
          {
            headers: {
              Authorization: user?.accessToken,
...
if(user.user.roleId==2 ){
  ...
  const addDocument = async () => {
    try {
      ...
      const response = await axios.post(
        `https://54.81.250.98:8083/api/documents/?projectCode=${projectCode}&areaCode=${areaCode}&deptSuffix=${deptSuffix}`,        
        formData,
        {
          headers: {
            Authorization: user?.accessToken,
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `https://54.81.250.98:8083/api/documents?companyId=${user?.user?.companyId}&assignedBy=${user.user.roleId}&userId=${user.user.id}&department=${user.user.departmentId}`,
        {
          headers: {
            Authorization: user?.accessToken,
            // Add other headers if needed
...
const assignDoc = async(assignedEmployees,myrecord)=>{
  try {
    ...
    const response = await axios.put(
      `https://54.81.250.98:8083/api/documents/?assignedTo=${assignedEmployees}&assignedBy=${user.user.roleId}&assignedFrom=${user.user.id}&docName=${myrecord.title}`,
      {},
      {
        headers: {
          Authorization: user?.accessToken,

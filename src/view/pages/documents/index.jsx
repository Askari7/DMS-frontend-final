  ...
  const fetch = async () => {
    try {
      ...
      for (const item of allUsers) {
        userOption.push({value:item.id, label:item.firstName});
      }
      for (const item of departments) {
        departmentOption.push({ value: item?.suffix, label: item?.suffix });
      }
      for (const item of projects) {
        projectOption.push({ value: item?.code, label: item?.code });
      }

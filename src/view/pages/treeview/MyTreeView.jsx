...
//     // Initialize MDR code in newData if present and not already initialized
//     if (info.title && !newData[info.projectCode][info.title]) {
//       newData[info.projectCode][info.title] = {};
//     }
...
//         if (trimmedTitle.includes(trimmedSuffix)) {
//           // Push document data to department array
//           newData[info.projectCode][info.title][trimmedDepartmentName].push({
//             title: document.docTitle,
//             docTitle: trimmedTitle,
//             status: document.status,
//             version:document.version
...
const processData = () => {
  ...
  const classes = useStyles();
  const BACKEND_URL = "http://127.0.0.2:8083"; // Update with your backend URL
 ...
 allowed='true';
     ...
     // Redirect to the external URL
      window.location.href = `http://127.0.0.2:3001/react-pdf-highlighter/?docName=${docName}.pdf&url=${url}&allowed=${allowed}&user=${user.user.roleId} ${user.user.firstName}`;
  };
  const fetchAppRev = async (title) => {
    try {
      const response = await axios.get(
        `http://127.0.0.2:8083/api/documents/establishment?companyId=${user?.user?.companyId}&userId=${user.user.id}&docName=${title}`,
        {
          headers: {
            Authorization: user?.accessToken,
            // Add other headers if needed

...
// import {Button,Form,Row,Col,Space,Skeleton ,Table,Input,Modal,message,Checkbox,Select,Upload,Avatar,Tag} from "antd";
...
//       console.log(roleOptions[role],email,firstName,lastName,selectedDepartments,"Ye ha asal data");
...
//     const roleOrder = ['Head of Department', 'Senior Engineer', 'Junior Engineer', 'Designer/Draugthsmen',"Client"];
...
export default function Users() {
  ...
  const fetch = async () => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8083/api/users/data?companyId=${user?.user?.companyId}`,
        {
          headers: {
            Authorization: user?.accessToken,
            // Add other headers if needed
      ...
      for (const item of response?.data.data.data) {
        ...
        // Assuming item has a departments array
        departmentOption[item.id] = item.title;
      }
  
      const allUsers = response.data.data.data.reduce((acc, item) => {
      ...
      const allUsersWithDepartment = allUsers.map(user => ({
        ...user,
        department: departmentOption[user.departmentId] || 'Unknown', // Default to 'Unknown' if departmentId is not found
      }));
  
      // Get the owner and add the department key
      const owner = {
        ...response.data.data.owner,
        department: departmentOption[response.data.data.owner.departmentId] || null,
      };
      ...
      const departmentUsers = allUsersWithOwner.filter(item=>item.departmentId == user.user.departmentId)
      console.log(departmentOption, "department options");
      setDepartmentOptions(options);
      
      if (user?.user.roleId == 2 || user?.user.roleId == 3 || user?.user.roleId == 4 || user?.user.roleId == 5) {
        setData(departmentUsers);
      } else {
        setData(allUsersWithOwner);
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "roleId",
      key: "roleId",
      filters: Object.keys(roleOptions).map(key => ({
        text: roleOptions[key],
        value: roleOptions[key]
      })),
      onFilter: (value, record) => roleOptions[record.roleId] === value,
      render: (text, record) => roleOptions[record.roleId]
    },
    {
      title: "Reporting To",
      dataIndex: "reported_to",
  ...
  <div style={{ overflowX: 'auto', width: '1300px' }}>
        {user?.user?.roleId === 2 && users ? (
          ...
          <OrganizationChart employees={data} />
        )}

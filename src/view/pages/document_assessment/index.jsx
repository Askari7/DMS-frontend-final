...
// }, [record]);
  ...
  const sendEmail = async()=>{
   ...
   allowed='true';
       ...
       const myUrl = `http://54.81.250.98:3001/react-pdf-highlighter/?docName=${docName}.pdf&url=${url}&allowed=${allowed}&user=${user.user.roleId} ${user.user.firstName}`;
     
    try {
      const response = await axios.post(
        `http://54.81.250.98:8083/api/clients/send-email-client`,
        {
  ...
  //       `http://54.81.250.98:8083/api/projects?companyId=${user?.user?.companyId}&clientStatus=fetchClients`
  ...
  const fetchClients = async()=>{
    try {
      ...
      const response = await axios.get(
        `http://54.81.250.98:8083/api/clients?companyId=${user?.user?.companyId}&recordId=${record}`,
        {
          headers: {
            Authorization: user?.accessToken,
          },
 ...
 allowed='true';
  ...
  const fetch = async()=>{
    try {
      const establishment = await axios.post(`http://54.81.250.98:8083/api/documents/establishment`,{companyId:user?.user.companyId},
      {
        headers: {
          Authorization: user?.accessToken,
        },
    ...
    try {
      if(rejectionMarks ){
        ...
        const response = await axios.put(
          `http://54.81.250.98:8083/api/documents/establishment?yourRole=${myrecord.yourRole}
        ...
        const response = await axios.put(
          `http://54.81.250.98:8083/api/documents/establishment?yourRole=${myrecord.yourRole}&record=${record}`,
          {
            revStatusArr: statusForRev.join(','), // Convert array to string
            appStatusArr: statusForApp.join(','), // Convert array to string
            docName: myrecord.docName,

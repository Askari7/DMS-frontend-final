...
import jsondata from './data.json';
  ...
  const MdrTemplate = () => {
    ...
    const reviewer = params.get('reviewer');
    const record = params.get('record');
    const allProjects = JSON.parse(params.get('allProjects'));    
    ...
    const [user, setUser] = useState(JSON.parse(localStorage?.getItem("user")));
    const [code,setCode] = useState()
    const dataArrayWithIndexes = Object.entries(jsondata).map(([index, value]) => ({  ...value, index: parseInt(index), checked: false, document:value.document.replace("xxxx",projectCode)}));
    const [data, setData] = useState(Object.values(dataArrayWithIndexes));
    const [documentCounts, setDocumentCounts] = useState({});
    const [information,setInformation] = useState([])
    const [documentInfo, setDocumentInfo] = useState([]);
    ...
    const templateModalShow = () => {
      ...
      selectedRows.forEach((index, sequenceNumber) => {
        ...
        // console.log(count, "counting");        
        for (let i = 0; i < count; i++) {
          let newDocument = data[index].document.replace('00X', (i + 1).toString().padStart(3, '0')).replace("undefined",projectCode)
          // console.log("document",newDocument);
          // Save the document name in allInfos array
          if (!allInfos[index]) {
            allInfos[index] = {};
        ...
        selectedRows.forEach(async (index) => {
          ...
          for (let i = 0; i < count; i++) {
            try {
              ...
              const responseDoc = await axios.post(
                `http://127.0.0.1:8083/api/documents/?count=${count}&documentInfo=${documentInfo}&information=${information}&version=000`,
                {
                  title,
                  startedDate,
                  expectedEndedDate,
